...
namespace SupportAPI.API.Comment
{
    partial class CommentController
    {
        ...
        public async Task<List<VMs.Comment>> GetCommentData(int ticketId)
        {
            ...
            {
                // QUERY
                List<int> dataIDComment = await context.TicketUsers
                .Where(x => x.RowStatus == Base.enRowStatus.Active && x.TicketId == ticketId)
                .Select(x => x.Id)
                .ToListAsync();

                List<Data.Comment> comments = await context.Comments
                    .Where(x => x.RowStatus == Base.enRowStatus.Active && dataIDComment.Contains(x.TicketUserId))
                    .ToListAsync();
